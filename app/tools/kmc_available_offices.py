    ...
    async def get_available_offices(self, city: str, capacity: int) -> Dict[str, Any]:
        ...
        try:
            with pyodbc.connect(self.connection_string) as conn:
                ...
                cursor.execute("EXEC sp_GetAvailableOffices @City = ?, @Capacity = ?", 
                             (city, capacity))
                ...
                logger.info(f"Successfully retrieved available offices. Found {len(results)} spaces in {city}")
                return response
                
        except pyodbc.Error as e:
            logger.error(f"Database error occurred while fetching available offices: {str(e)}")
            raise ValueError(f"Database query failed: {str(e)}")
        except Exception as e:
            logger.error(f"Unexpected error in get_available_offices: {str(e)}", exc_info=True)
